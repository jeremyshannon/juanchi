...
function rcbows.register_arrow(name, def)
		...
		on_step = function(self, dtime)
			...
			while thing do
				...
				elseif thing.type == "node" then
					...
					--minetest.chat_send_all("draw:"..node_name)
					...
					if minetest.registered_items[node_name].walkable then
						if not(def.embed) then
							...
							local embed_node_name = def.inventory_arrow.name.."_embed"
							--Calculate the arrow rotation
							local param2
							local arrow_pos
							local pos_above
							local node_above_drawtype
							if thing.under then
								pos_above = {
									x= thing.under.x,
									y= thing.under.y+1,
								...
								node_above_drawtype = minetest.registered_nodes[minetest.get_node(pos_above).name].drawtype or "normal"
								--minetest.chat_send_all(node_above_drawtype)
							end
							--local round_pos = vector.round(pos)
							--minetest.chat_send_all(tostring(round_pos.x)..","..tostring(round_pos.y)..","..tostring(round_pos.z))
							--local round_above_pos = vector.round(thing.above)
							--minetest.chat_send_all(tostring(round_above_pos.x)..","..tostring(round_above_pos.y)..","..tostring(round_above_pos.z))
							--local old_pos_drawtype = minetest.get_node(old_pos).drawtype or "normal"
							--local round_old_pos = vector.round(thing.above)
							--minetest.chat_send_all(tostring(round_old_pos.x)..","..tostring(round_old_pos.y)..","..tostring(round_old_pos.z))
							if thing.under and node_above_drawtype and node_above_drawtype =="airlike" then
								arrow_pos = pos_above
								param2 = 0
								--minetest.chat_send_all("test")
							else
								arrow_pos = self.old_pos
								local arrow_dir = vector.normalize(vector.direction(self.old_pos, pos))
								param2 = minetest.dir_to_facedir(arrow_dir)
								--minetest.chat_send_all(tostring(param2))
							end
							minetest.set_node(arrow_pos, {name = embed_node_name, param2 = param2+2})
						end
						self.waiting_for_removal = true
						self.object:remove()
	...
	if def.embed then
		minetest.register_node(def.inventory_arrow.name.."_embed", {
			drawtype = "nodebox",
			paramtype = "light",
			paramtype2 = "facedir",
			tiles = {"farbows_arrow_stick.png"},
			node_box = {
				type = "fixed",
				fixed = {
					{-0.25, -0.5, 0, 0.25, 0, 0}, -- NodeBox1
					{0, -0.5, -0.25, 0, 0, 0.25}, -- NodeBox2
				}
			},
			drop = def.inventory_arrow.name,
